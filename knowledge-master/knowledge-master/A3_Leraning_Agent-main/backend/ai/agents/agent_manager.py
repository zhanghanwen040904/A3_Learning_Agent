...
class AgentManager:
    ...
    def _build_context(
        ...
        if stored_profile:
            profile = {field: stored_profile.get(field) or "待进一步观察" for field in self.PROFILE_FIELDS}
        else:
    ...
    def run_pipeline(
        ...
        if dialogue_text and not content_audit(dialogue_text):
            result["errors"].append("学生输入未通过内容安全审核")
            trace("SafetyAgent", "failed", "学生输入未通过内容安全审核")
            return result
        ...
        trace("ProfileAgent", "completed", "已合并学生画像与本次学习需求", int((time.perf_counter() - start) * 1000))
        ...
        result["plan"] = plan
        trace("PlannerAgent", "completed", "已规划六类互补学习资源", int((time.perf_counter() - start) * 1000))
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            futures = {executor.submit(generate_one, resource_type): resource_type for resource_type in self.resource_agents}
        ...
        trace("SafetyAgent", "completed" if result["safety"].get("passed") else "failed", result["safety"].get("risk", ""))
