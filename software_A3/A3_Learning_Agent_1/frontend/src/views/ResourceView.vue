    ...
    <el-card class="panel catalog-panel">
      <template #header>
        <div class="header-line">
          ...
          <el-tag :type="displayMode === 'current' ? 'primary' : displayMode === 'history' ? 'warning' : 'info'" effect="plain">
            {{ displayModeLabel }}
          </el-tag>
        </div>
      </template>
...
function eventType(status) {
  return ({ completed: "primary", warning: "warning", failed: "danger" }[status] || "primary");
}
