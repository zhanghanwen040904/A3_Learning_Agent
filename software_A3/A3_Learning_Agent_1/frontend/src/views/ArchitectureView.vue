<template>
  <div class="page agent-market-page">
    ...
    <el-card ref="workflowRef" class="panel">
      <template #header><div class="section-head"><div><strong>原生智能体协同工作流</strong><p>用户只表达目标，系统自动组织多个角色完成任务拆解、工具调用、质量审核和学习反馈。</p></div><el-tag type="primary" effect="plain">可审计链路</el-tag></div></template>
      <div class="workflow-grid">
        <div v-for="flow in workflows" :key="flow.title" class="workflow-card"><b>{{ flow.index }}</b><h3>{{ flow.title }}</h3><p>{{ flow.desc }}</p><div class="chips"><el-tag v-for="tag in flow.tags" :key="tag" type="primary" effect="plain" size="small">{{ tag }}</el-tag></div></div>
      </div>
    </el-card>
    ...
    <el-card class="panel">
      ...
      <el-timeline>
        <el-timeline-item v-for="item in requirements" :key="item.title" type="primary"><h3>{{ item.title }}</h3><p>{{ item.desc }}</p><el-tag v-for="tag in item.tags" :key="tag" class="tag" type="primary" effect="plain">{{ tag }}</el-tag></el-timeline-item>
      </el-timeline>
    </el-card>
...
const agents = [
  ["ProfileAgent","PA","学习画像分析师","画像与理解","自然语言对话、学习历史","六维动态画像、追问问题",["Spark Chat","Content Audit","MySQL"],"通过多轮对话抽取专业、课程、基础、薄弱点、目标和偏好，形成学习记忆。","支撑对话式画像构建。"],
  ...
  ["DocumentAgent","DA","课程文档生成师","资源生产","画像、计划、教材片段","递进式课程讲解",["Spark Chat","RAG"],"生成学习目标、概念讲解、案例、易错点和自测问题。","生成可直接学习的课程文档。"],
  ["QuizAgent","QA","分层练习设计师","资源生产","画像、知识点","基础/提升/应用练习",["Spark Chat","Rubric"],"围绕薄弱点设计分层题目、参考答案和解析。","构建练习资源和评估入口。"],
  ["ReadingAgent","RD","拓展阅读策展师","资源生产","课程知识、专业目标","导读、问题链、探索方向",["Spark Chat","RAG"],"组织可信拓展阅读，连接课程知识、专业场景与前沿应用。","提升资源广度。"],
  ["MindMapAgent","MM","知识可视化设计师","资源生产","薄弱点、教材片段","Mermaid思维导图",["Mermaid","Spark Chat"],"把概念、原理、步骤、案例和易错点组织成可渲染知识结构图。","提供可视化支架。"],
  ["CodeAgent","CA","代码实操生成师","资源生产","课程知识点","Python实操案例",["Python Check","Spark Chat"],"将抽象原理转化为代码实验案例。","支撑实践型学习。"],
  ...
  ["TutorAgent","TA","智能辅导老师","答疑与反馈","学生问题、知识库","解释、图解、自测、来源",["RAG","Spark Chat"],"基于课程知识库即时答疑并给出进一步练习建议。","提供智能辅导。"],
  ["PackagerAgent","PK","资源结果编排师","答疑与反馈","合格资源、质量结果","统一资源包、协作证据",["Trace","Metadata"],"把多智能体结果组织为可展示、可追踪、可审计的资源集合。","便于展示协作链路。","概念组件","info"],
].map((item) => ({ name: item[0], short: item[1], role: item[2], category: item[3], input: item[4], output: item[5], tools: item[6], desc: item[7], value: item[8], status: item[9] || "已启用", type: item[10] || "info" }));
...
.tabs button.active { color: #fff; border-color: #2563eb; background: linear-gradient(135deg,#2563eb,#4f46e5); }
...
.avatar { display: grid; width: 44px; height: 44px; flex: 0 0 auto; place-items: center; border-radius: 15px; color: #fff; background: linear-gradient(135deg,#2563eb,#4f46e5); font-weight: 800; }
