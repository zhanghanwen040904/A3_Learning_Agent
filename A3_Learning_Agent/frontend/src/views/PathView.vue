          ...
          <div class="body"><div class="section"><b>本阶段配套资源</b><span>保留个性化说明与质量审核报告</span></div><el-tabs v-if="s.resources.length" v-model="activeResourceTabs[i]" class="resource-tabs" stretch @tab-change="renderMarkmaps" @tab-click="renderMarkmaps">
                <el-tab-pane v-for="r in s.resources" :key="rid(r)" :name="rid(r)">
                  ...
                  <template v-else-if="isVideo(r)">
                    <div class="video-resource" :class="{collapsed:!resourceOpen(r)}">
                      <video v-if="primaryVideoUrl(r)" class="learning-video" :src="primaryVideoUrl(r)" controls preload="metadata"></video>
                      <div v-else class="video-placeholder">暂未找到可播放的视频文件</div>
                      <div class="video-list" v-if="resourceVideos(r).length>1">
                        <b>推荐视频</b>
                        <a v-for="v in resourceVideos(r)" :key="v.id||v.url" :href="absoluteVideoUrl(v.url)" target="_blank" rel="noopener">{{v.title||v.file_name}}</a>
                      </div>
                      <div class="res-content video-script" v-if="resourceText(r)">{{resourceText(r)}}</div>
                    </div>
                  </template>
...
function apiOrigin(){return (import.meta.env.VITE_API_BASE_URL||'http://localhost:5000/api').replace(/\/api\/?$/,'')}
function absoluteVideoUrl(url){if(!url)return'';if(/^https?:\/\//i.test(url))return url;return `${apiOrigin()}${String(url).startsWith('/')?url:`/${url}`}`}
function resourceVideos(r){const meta=r.metadata||{};const videos=Array.isArray(meta.videos)?meta.videos:[];const videoUrl=r.video_url||meta.video_url||meta.videoUrl;if(videoUrl&&!videos.some(item=>item.url===videoUrl))videos.unshift({title:r.title,url:videoUrl});return videos.filter(item=>item&&item.url).map(item=>({...item,url:absoluteVideoUrl(item.url)}))}
function primaryVideoUrl(r){return resourceVideos(r)[0]?.url||''}
